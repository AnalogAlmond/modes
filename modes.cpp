...
int main(){
    ...
    if(find(begin(keys), end(keys), key)!=end(keys)){
        for(int i = 0; i < MODES; i++){
            used[i] = r;
            cout << mode[r] << '\n';
            command = "mocp --enqueue " + key + '/' + mode[r] + ".mp3";//command to get the file into the queue
    ...
    //if the user wants multiple keys:
        ...
        for(int i = 0; i < scales; i++){
           ...
           cout << usedcombos[i] << '\n';
           command = "mocp --enqueue " + usedcombos[i] + ".mp3";//command to get the file into the queue
           system(command.c_str());//turns it into a constant char* and runs the command
           if(i == (scales-1)) break;
